...
class SmartTourismRecommender:
    ...
    def format_recommendation(self, row: pd.Series) -> dict:
        """Format rekomendasi menjadi dictionary - DIPERBARUI"""
        nama = row['nama']
        ...
        # ✅ PERBAIKAN: Definisikan lokasi_data dari row
        lokasi_data = row.get('lokasi', {})
        # Format lokasi
        ...
        # Jika maps_link masih kosong, coba dari kolom 'maps'
        if not maps_link and 'maps' in row:
            maps_link = row['maps']
        ...
        # Dapatkan fasilitas
        fasilitas_str = ""
        if 'fasilitas_list' in row and isinstance(row['fasilitas_list'], list):
            if row['fasilitas_list']:
                fasilitas_str = ", ".join([str(f).title() for f in row['fasilitas_list'][:5]])
